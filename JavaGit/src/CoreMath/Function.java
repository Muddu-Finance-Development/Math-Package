...
public final class Function {
    ...
    static double vals[] = {76.18009172947146, -86.50532032941677, 24.01409824083091, -1.231739572450155, 0.1208650973866179e-2, -0.5395239384953e-5};// Lanczos coefficient values

    protected static double lgamma(double x)//returns the log of the values.. for large values
    {
        return x > 1 ? Math.log(x + 5.5) * (x + 0.5) - (x + 5.5) + Math.log(coeffs(x) * Math.sqrt(2 * Math.PI) / x) : (x <= 1.0 && x > 0 ? lgamma(x + 1) - Math.log(x) : (x < 0 ? Math.log(reflect(x)) : Double.NaN));
    }
    ...
    public static double binom(int n, int k) {
        ...
        return Math.floor(Math.exp(lgamma(n + 1.0) - lgamma(n - k + 1.0) - lgamma(k + 1.0)));
    }
