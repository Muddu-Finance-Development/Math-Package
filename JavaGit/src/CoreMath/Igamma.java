 ...
 * @author Muddu
 */
public class Igamma extends ContFract {
    ...
    public double errf(double xerr) {//error function for all x values positive and negative//
        double aconst;
        double xvar;
        double sign = xerr;
        if (xerr == 0) {
    ...
    protected void computeFract(int n) {
        b += 2;// auxillary bn=x-a+2n for n=0....... x+2,x+4,x+6 etcí//
        vars[0] = n * (a - n);
        vars[1] = b;
        return;
    }
