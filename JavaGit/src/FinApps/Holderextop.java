 ...
 * @author Muddu
 */
public class Holderextop {

    public Holderextop(double adpremium, double strikeadj, double newtime, double yield, double rate) {
        ap = adpremium;
        x2 = strikeadj;
        t2 = newtime;
        crate = yield;
    ...
    public void parAms(double s, double x, double volatility, double t) {
        ...
        y2 = (Math.log(s / sval) + (brate + (sigma * sigma) * 0.5) * (time)) / (sigma * Math.sqrt(time));
        y = (Math.log(s / sval2) + (brate + (sigma * sigma) * 0.5) * (time)) / (sigma * Math.sqrt(time));
        z = (Math.log(s / x2) + (brate + (sigma * sigma) * 0.5) * (t2)) / (sigma * Math.sqrt(t2));
        z2 = (Math.log(s / strike) + (brate + (sigma * sigma) * 0.5) * (time)) / (sigma * Math.sqrt(time));
        rho = (Math.sqrt(time / t2));
    }

    public double extCall(double s, double x, double volatility, double t) {
        typeofop = 0;
        ...
        double n1 = (Bivnorm.bivar_params.evalArgs((y2 - sigma * Math.sqrt(t)), (z - sigma * Math.sqrt(t2)), rho));
        double n2 = (Bivnorm.bivar_params.evalArgs((y - sigma * Math.sqrt(t)), (z - sigma * Math.sqrt(t2)), rho));
        double n3 = (Bivnorm.bivar_params.evalArgs((y2 - sigma * Math.sqrt(t)), -10.0, rho));
        double n4 = (Bivnorm.bivar_params.evalArgs((y - sigma * Math.sqrt(t)), -10.0, rho));
        double nn = (n1 - n2 - n3 + n4);
        double prob1 = (p.ncDisfnc(z2) - p.ncDisfnc(y));
        double prob2 = (p.ncDisfnc(z2 - sigma * Math.sqrt(time)) - p.ncDisfnc(y - sigma * Math.sqrt(time)));
        double prob3 = (p.ncDisfnc(y2 - sigma * Math.sqrt(time)) - p.ncDisfnc(y - sigma * Math.sqrt(time)));
        b.bscholEprice(s, strike, sigma, time, r);
        double cval = b.getCalle();
        double call = (cval + s * Math.exp((brate - r) * t2) * mm - x2 * Math.exp(-r * t2) * nn - s * Math.exp((brate - r) * time) * prob1 + strike * Math.exp(-r * time) * prob2 - ap * Math.exp(-r * time) * prob3);
        return call;
    }
    ...
    public double extPut(double s, double x, double volatility,
        ...
        double prob2 = (p.ncDisfnc(y2 - sigma * Math.sqrt(time)) - p.ncDisfnc(z2 - sigma * Math.sqrt(time)));
        double prob3 = (p.ncDisfnc(y2 - sigma * Math.sqrt(time)) - p.ncDisfnc(y - sigma * Math.sqrt(time)));
        b.bscholEprice(s, strike, sigma, time, r);
        double pval = b.getPute();
        double put = (pval - s * Math.exp((brate - r) * t2) * mm + x2 * Math.exp(-r * t2) * nn + s * Math.exp((brate - r) * time) * prob1 - strike * Math.exp(-r * time) * prob2 - ap * Math.exp(-r * time) * prob3);
        return put;
    }
