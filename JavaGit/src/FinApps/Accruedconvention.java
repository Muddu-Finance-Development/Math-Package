...
public class Accruedconvention
{
	...
	switch(flagvalue)
		...
		{
                    samedays=(nextcoupondate.get(Calendar.DATE)-settlementdate.get(Calendar.DATE));
                    for(int n=(previouscoupon.get(Calendar.MONTH)+1);n<nextcoupondate.get(Calendar.MONTH);n++)
                    {
			temp.set(Calendar.MONTH,n);
			actualdays+=temp.getActualMaximum(Calendar.DAY_OF_MONTH);
                    }
                    actualdays+=(previouscoupon.getActualMaximum(Calendar.DAY_OF_MONTH)-previouscoupon.get(Calendar.DATE));
                    setPreviousCouponDays((double)(actualdays-samedays)/(actualdays));
                    return (double)samedays/actualdays;
                }
		int setdays= (settlementdate.getActualMaximum(Calendar.DAY_OF_MONTH)-settlementdate.get(Calendar.DATE));
		actualday=setdays;
		for(int i=(settlementdate.get(Calendar.MONTH)+1);i<nextcoupondate.get(Calendar.MONTH);i++)
                {
		...
		return (double)actualday/actualdays; //returns fraction of the
            ...
            case 2:
		for(int n=(previouscoupon.get(Calendar.MONTH)+1);n<settlementdate.get(Calendar.MONTH);n++)
                {
                    temp.set(Calendar.MONTH,n);
                    actualdays+=temp.getActualMaximum(Calendar.DAY_OF_MONTH);
		}
		...
		actualday= (settlementdate.getActualMaximum(Calendar.DAY_OF_MONTH)-settlementdate.get(Calendar.DATE));
                for(int i=(settlementdate.get(Calendar.MONTH)+1);i<nextcoupondate.get(Calendar.MONTH);i++)
                {
                    temp.set(Calendar.MONTH,i);
                    actualday+=temp.getActualMaximum(Calendar.DAY_OF_MONTH);
		...
		//Actual/365 or 366 in leap year (eg,
		{
                    samedays=(nextcoupondate.get(Calendar.DATE)-settlementdate.get(Calendar.DATE));
                    int total;
                    total=(previouscoupon.getActualMaximum(Calendar.DAY_OF_YEAR)|nextcoupondate.getActualMaximum(Calendar.DAY_OF_YEAR))==366?366:365;
		...
		actualday+=nextcoupondate.get(Calendar.DATE);
		int totaldays;
		totaldays=(previouscoupon.getActualMaximum(Calendar.DAY_OF_YEAR)|nextcoupondate.getActualMaximum(Calendar.DAY_OF_YEAR))==366?366:365;
                System.out.println("Actual days between coupon and settlement =="+actualdays);
		...
		return (double)actualday/(360.0);
			...
			return (double)samedays/(360.0/coupons);
                    ...
                    int couponsettle;
                    int daysettle=settlementdate.getActualMaximum(Calendar.DAY_OF_MONTH);
