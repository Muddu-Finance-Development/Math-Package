 ...
 * @author Muddu
 */
public class Blackscholecp {

    public Blackscholecp() {
    }
     ...
     *
     */
    public Blackscholecp(double carryrate) {
    ...
    public void bscholEprice(double sprice, double strike, double volatility, double time, double rate) {
        ...
        setpute(((strike * Math.exp(-rate * time)) * p.ncDisfnc(-d2)) - (sprice * Math.exp((brate - rate) * time)) * p.ncDisfnc(-d1));
        ...
        thetac = (((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility) / (2 * Math.sqrt(time))) - thetaterm1 - thetaterm2);
    }
    ...
    private void dvalues(double sprice, double strike, double volatility, double time, double rate) {
        ...
        d1 = ((Math.log(sprice / strike) + (brate + (volatility * volatility) * 0.5) * time) / (volatility * Math.sqrt(time)));
    ...
    public void setTheta(double sprice, double strike, double volatility, double time, double rate) {
        ...
        double thetaterm3 = ((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility) / (2 * Math.sqrt(time)));
        ...
        double thetatermc = ((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility) / (2 * Math.sqrt(time)));
