 ...
 * @author Muddu
 */
public class Exonexop {

    public Exonexop(double rate, double yield1, double yield2, double sig1, double sig2, double p) {
        r = rate;
        crate1 = yield1;
        crate2 = yield2;
        sigma1 = sig1;
    ...
    private void params(double s1, double s2, double time1, double time2, double q) {
        double pratio;
        t1 = time1;
        t2 = time2;
        qval = q;
        ...
        d1 = ((Math.log(s1 / (pratio * s2)) + (b1 - b2 + (sigma * sigma) * 0.5) * t1) / (sigma * Math.sqrt(t1)));
        d2 = (d1 - sigma * Math.sqrt(t1));
        d3 = ((Math.log((pratio * s2) / s1) + (b2 - b1 + (sigma * sigma) * 0.5) * t1) / (sigma * Math.sqrt(t1)));
        d4 = (d3 - sigma * Math.sqrt(t1));
    }

    public double exchqfs(double s1, double s2, double time1, double time2, double q) {
        typeop = 1;
        sv1 = s1;
        sv2 = s2;
        ...
        double c = (s1 * Math.exp((b1 - r) * t2) * m1 - s2 * Math.exp((b2 - r) * t2) * m2 - qval * s2 * Math.exp((b2 - r) * t1) * n);
        return c;
    }
    ...
    public double exchsfq(double s1, double s2, double time1, double time2, double q) {
        ...
        double c = (s2 * Math.exp((b2 - r) * t2) * m1 - s1 * Math.exp((b1 - r) * t2) * m2 + qval * s2 * Math.exp((b2 - r) * t1) * n);
        return c;
    }

    public double exchqfs2(double s1, double s2, double time1, double time2, double q) {
        typeop = 0;
        sv1 = s1;
        sv2 = s2;
        params(s1, s2, time1, time2, q);
    ...
    public double exchs1fs2(double s1, double s2, double time1, double time2, double q) {
        typeop = 0;
        sv1 = s1;
        sv2 = s2;
        params(s1, s2, time1, time2, q);
    ...
    private class i1 extends NewtonRaphson {
        ...
        public double pars() {
            double seedval = (sv1 * Math.exp((b1 - r) * (t2 - t1)) / sv2 * Math.exp((b2 - r) * (t2 - t1)));
        ...
        public double newtonroot(double rootvalue) {
            double solution = 0.0;
            double z1 = ((Math.log(rootvalue) + ((t2 - t1) * (sigma * sigma) * 0.5)) / (sigma * Math.sqrt(t2 - t1)));
    ...
    private class i2 extends NewtonRaphson {
        ...
        public double pars() {
            double seedval = (sv2 * Math.exp((b2 - r) * (t2 - t1)) / sv1 * Math.exp((b1 - r) * (t2 - t1)));
            double sval2 = newtraph(seedval);
            return sval2;
        }
