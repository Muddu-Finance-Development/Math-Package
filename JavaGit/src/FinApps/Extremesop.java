 ...
 * @author Muddu
 */
public class Extremesop {

    public Extremesop(double rate, double yield, double eta) {
        //eta=1=call, -1 = put
        e = eta;
        r = rate;
        crate = yield;
    ...
    public double extremeSp(double s, double s2, double t1, double t2, double vol) {
        ...
        double w = e * (s * Math.exp((b - r) * t2) * term1 * term2 - Math.exp(-r * (t2 - t1)) * s * Math.exp((b - r) * t2) * term1 * term3 + Math.exp(-r * t2) * s2 * term4 - Math.exp(-r * t2) * s2 * sig / (2.0 * b) * Math.exp(2.0 * mu1 * m / sig)
                * term5 - Math.exp(-r * t2) * s2 * term6 + Math.exp(-r * t2) * s2 * sig / (2.0 * b) * Math.exp(2.0 * mu1 * m / sig) * term7);
        return w;
    }

    public double extremeSprev(double s, double s2, double t1, double t2, double vol) {
        Probnorm p = new Probnorm();
        double sig = (vol * vol);
        mu1 = (b - sig * 0.5);
        mu = (mu1 + sig);
        ...
        double w = -e * (s * Math.exp((b - r) * t2) * term1 * term2 + Math.exp(-r * t2) * s2 * term3 - Math.exp(-r * t2) * s2 * sig / (2.0 * b) * Math.exp(2.0 * mu1 * m
                / sig) * term4 - s * Math.exp((b - r) * t2) * term1 * term5 - Math.exp(-r * (t2 - t1)) * s * Math.exp((b - r) * t2) * (1.0 - sig / (2.0 * b)) * term6);
        return w;
    }
