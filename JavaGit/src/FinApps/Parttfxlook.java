 ...
 * @author Muddu
 */
public class Parttfxlook {

    public Parttfxlook(double rate, double yield) {
        crate = yield;
        r = rate;
    ...
    private void params(double s, double x, double t1, double t2, double sigma) {
        sig = (sigma * sigma);
    ...
    public double partFxCall(double s, double x, double t1, double t2, double sigma) {
        ...
        double m1 = (Bivnorm.bivar_params.evalArgs(fac1, fac2, (-Math.sqrt(t1 / t2))));
        double m2 = (Bivnorm.bivar_params.evalArgs(e1, d1, (Math.sqrt(1.0 - t1 / t2))));
        double m3 = (Bivnorm.bivar_params.evalArgs(-e1, d1, (-Math.sqrt(1.0 - t1 / t2))));
        double m4 = (Bivnorm.bivar_params.evalArgs(f2, -d2, (-Math.sqrt(t1 / t2))));
        double term = fac3 * m1 + Math.exp(brate * t2) * m2;
        double factor1 = s * Math.exp((brate - r) * t2) * n1 - Math.exp(-r * t2) * x * n2 + s * Math.exp(-r * t2) * sig / (2.0 * brate) * term;
        double factor2 = -s * Math.exp((brate - r) * t2) * m3 - x * Math.exp(-r * t2) * m4 + Math.exp(-brate * (t2 - t1)) * (1.0 - sig / (2.0 * brate)) * s * Math.exp((brate - r) * t2) * n3 * n4;
    ...
    public double partFxPut(double s, double x, double t1,
        ...
        double m4 = (Bivnorm.bivar_params.evalArgs(-f2, d2, (-Math.sqrt(t1 / t2))));
        double term = fac3 * m1 - Math.exp(brate * t2) * m2;
        double factor1 = x * Math.exp(-r * t2) * n2 - s * Math.exp((brate - r) * t2) * n1 + s * Math.exp(-r * t2) * sig / (2.0 * brate) * term;
        double factor2 = s * Math.exp((brate - r) * t2) * m3 + x * Math.exp(-r * t2) * m4 - Math.exp(-brate * (t2 - t1)) * (1.0 - sig / (2.0 * brate)) * s * Math.exp((brate - r) * t2) * n3 * n4;
        double c = (factor1 + factor2);
        return c;
    }
